    ...
    <div>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr v-for="tiket in tiketList" :key="tiket.id">
                            ...
                            <td>{{ tiket.status }}</td>
                            <td>
                                <button class="btn btn-primary me-2" @click="editTiket(tiket)">Edit</button>
                                <button class="btn btn-danger" @click="deleteTiket(tiket.id)">Hapus</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <VForm class="card mb-10" @submit="submit" :validation-schema="formSchema">
            <div class="card-header align-items-center">
                <h2 class="mb-0">{{ isEditMode ? 'Edit Tiket' : 'Buat Tiket Baru' }}</h2>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="fv-row mb-8">
                            <label class="form-label fw-bold fs-6 required">Judul Tiket</label>
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <Field class="form-control form-control-lg form-control-solid" type="textarea" name="deskripsi"
                                autocomplete="off" v-model="formData.deskripsi" />
                            <div class="fv-plugins-message-container">
                                <div class="fv-help-block">
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <Field class="form-control form-control-lg form-control-solid" as="select" name="status"
                                autocomplete="off" v-model="formData.status">
...
export default defineComponent({
    setup() {
        const tiketList = ref([
            ...
            { id: 1, judul: 'Tiket 1', deskripsi: 'Deskripsi Tiket 1', status: 'open' },
            { id: 2, judul: 'Tiket 2', deskripsi: 'Deskripsi Tiket 2', status: 'in_progress' },
        ]);
        ...
        const submit = () => {
            if (isEditMode.value) {
                ...
                const tiketIndex = tiketList.value.findIndex(tiket => tiket.id === formData.value.id);
                tiketList.value[tiketIndex] = { ...formData.value };
                toast.success('Tiket berhasil diupdate');
            } else {
                // Jika mode create, tambahkan tiket baru
                tiketList.value.push({ ...formData.value, id: tiketList.value.length + 1 });
